...
####Load tables####
{
  ####table des comptages cellules####
  comptages <- read_xlsx("~/Documents/Divers_labo/Projets_2_suivi/2019_07_MASTICELLS_DSCC/2023_lait_Pin_Haras_HF_NO/2023_03_28_Fluidigm/2023_comptages.xlsx")
...
#clean table to remove genes and samples with too many missing values####
  ...
  #identify samples corresponding to milk cells####
  {
  ...
  #comparaison lait seul
  analyse <- c("lait")
  quarter_to_keep <- quartier_lait
...
# table_log_fc$Cluster_4_samples[table_log_fc$Type == "Macro"] <- 5
# table_log_fc$Cluster_4_samples[table_log_fc$Type == "Neutro"] <- 6
# table_log_fc$Cluster_4_samples[table_log_fc$Type == "Lympho"] <- 7
...
table_log_fc_agg <- merge(table_analyse_agg,
                          table_log_fc[table_log_fc$duplicats==1,c("Quartier","Name", "Date",
                                                                   "Volume",
                                                                   "Echantillon","Type",
                                                                   "Extraction","Macrophage","Lymphocyte","Neutrophile",
                                                                   "Macro_num", 
                                                                   "Lympho_num", "Neutro_num",                                      
                                                                   "Race", "SCS_cate",                                         
                                                                   "SCS", "SCS_cate_2", "CCS",                                              
...
####heatmap####
{
  ...
  matrix_heatmap <- matrix_heatmap[,c(is.element(colnames(matrix_heatmap),clusters_pop_sep$Gene[is.element(clusters_pop_sep$Nom,c("Macro","Lc", "PMN2"))]))]
  
  
  
  # rownames(matrix_heatmap)<-paste0(table_analyse$Quartier,"_",table_analyse$Type, "_", table_analyse$Name)
  rownames(matrix_heatmap)<-table_analyse$Quartier
  
  # write.csv(matrix_heatmap, "matrix_log2FC_2023_03_28.csv", row.names = T, quote = F)
  ...
  annotation_row<-data.frame(
    Sample_clusters=factor(table_analyse$Cluster_4_samples),
